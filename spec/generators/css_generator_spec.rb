...
# The stylesheet ships COMMENTED OUT -- an explainer, then ONE block comment round
# the whole thing, so enabling is deleting two lines or one editor keystroke. Two
# properties are load-bearing and neither is visible by reading the file casually:
# that nothing in it is live as generated, and that enabling it the documented way
# yields CSS rather than wreckage. A single comment-closing delimiter written
# inside the block breaks both at once, and would leave part of the stylesheet
# live without anybody having enabled it.
RSpec.describe AuditLog::Generators::Views::CssGenerator do
  ...
  # THE DOCUMENTED GESTURE: delete the bare comment-opening line under the
  # explainer, and the file's last line. Anything else -- a second bare `/*`, a
  # stray closer inside the block -- makes this produce something other than the
  # stylesheet, which is what the examples below are for.
  def enable(css)
    lines = css.lines
    opener = lines.rindex { |l| l.chomp == "/*" }
    closer = lines.rindex { |l| l.chomp == "*/" }
    raise "no single wrapping block found" unless opener && closer && opener < closer

    lines[(opener + 1)...closer].join
  end

  it "wraps the whole stylesheet in exactly one block, under one explainer" do
    css = generate

    expect(css.lines.count { |l| l.chomp == "/*" }).to eq(1), "expected one CSS block"
    expect(css.lines.count { |l| l.chomp == "*/" }).to eq(2), "the explainer's, and the file's"
    expect(css.lines.first).to start_with("/*"), "the explainer opens the file"
    expect(css.lines.last.chomp).to eq("*/")
  end
