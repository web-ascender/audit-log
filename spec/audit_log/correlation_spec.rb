...
RSpec.describe "request correlation" do
  ...
  # config.correlated_databases decides who pays for the correlation round trip.
  ...
  describe "config.correlated_databases" do
    ...
    def uncorrelate_the_connection!
      ...
      allow(AuditLog.config).to receive(:correlated_databases).and_return([])
      AuditLog::Context.forget_stamp!(conn)
      conn
    end
